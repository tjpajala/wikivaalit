...
df = pd.read_csv(data_path+'/candidate_answers_utf8.csv', sep=';', encoding='utf-8', engine='python')
...
with open(data_path+"/categories.txt", 'rb') as cats_file:
    pages_to_retain = pd.read_csv(cats_file)
    pages_to_retain.columns=['page_id', 'category', 'page_type', 'page_title']

#with open(data_path+"/wiki_cats.pkl", 'rb') as meta_file:
...
id2word = gensim.corpora.Dictionary.load_from_text(data_path+'/orig_wiki/wiki_filtered_wordids.txt.bz2')
wiki_bow = gensim.corpora.MmCorpus(data_path+'/orig_wiki/wiki_filtered.mm')

with open(data_path+"/orig_wiki/wiki_filtered.mm.metadata.cpickle", 'rb') as meta_file:
...
if load_previous_lda_model:
    lda_elec = gensim.models.LdaModel.load(saved_models_path+'/lda_elec.lda')
else:
    ...
    lda_elec = gensim.models.LdaMulticore(corpus=wiki_bow, id2word=dictionary_elec, num_topics=n_topics,
                                          passes=passes, workers=1, random_state=123)
    if not test_run:
        lda_elec.save(saved_models_path+'/lda_elec.lda')
...
if not test_run:
    MmCorpus.serialize(data_path+"/elec_lda_corpus.mm",elec_lda_corpus)
...
if test_run:
    ...
    if load_previous_similarity:
        index = gensim.similarities.Similarity.load(saved_models_path+'/similarity_elec.index')
    else:
        index = gensim.similarities.Similarity(output_prefix="shard", corpus=wiki_bow, num_features=lda_elec.num_terms)
        index.save(saved_models_path+'/similarity_elec.index')
...
if not test_run:
    df.to_csv(data_path+'/cand_df_filled.csv')
    df.to_pickle(data_path+'/cand_df.pkl')
else:
    df.to_csv(data_path+'/cand_df_filled_test.csv')
    df.to_pickle(data_path+'/cand_df_test.pkl')
